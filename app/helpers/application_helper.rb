  ...
  def render_pagination(collection, options = {})
    ...
    begin
      ...
      if collection.next_page.present?
        html += content_tag(:div, link_to(t('more'), url_for(params.merge(:page => collection.next_page, id: collection.first.gallery.id )), :class => 'btn btn-lg btn-default', :role => 'more'), class: 'text-center')
      end
    rescue
      nil
    end
